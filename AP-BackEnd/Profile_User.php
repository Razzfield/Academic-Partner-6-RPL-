<?php
    ...
    if(mysqli_num_rows($query_vali)){
        $query_old = mysqli_query($conn, "SELECT email FROM user_mahasiswa WHERE id_mahasiswa='$id_mahasiswa' ");
        ...
        if($request){
            ...
            if(mysqli_num_rows($query_check) && $email!=$old_email){
                ...
                $query_register = mysqli_query($conn, "UPDATE user_mahasiswa SET email='$email',password='$encrypt_password',nama_lengkap='$nama_lengkap',tanggal_lahir='$tanggal_lahir', id_line ='$id_line', nomor_hp='$nomor_hp', nim='$nim', jurusan='$jurusan', fakultas='$fakultas', angkatan='$angkatan', minat='$minat' ");
